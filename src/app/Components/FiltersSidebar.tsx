    ...
    <>
      ...
      <div className="lg:hidden mb-6">
        <button
          ...
          className="w-full flex items-center justify-between bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-700 font-medium hover:border-orange-300 transition-colors"
        >
      ...
      <div className={`lg:block ${isOpen ? 'block' : 'hidden'} bg-white border border-slate-200 rounded-2xl p-6 shadow-sm h-fit`}>
        <div className="flex items-center justify-between mb-6">
          ...
          {(selectedCountries.length + selectedTuition.length + selectedRankings.length > 0) && (
            <button
              ...
              className="text-sm text-slate-500 hover:text-orange-500 transition-colors flex items-center gap-1"
            >
              <X size={14} />
              Clear all
            </button>
        ...
        {/* Apply Button */}
        <button className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white font-semibold py-3 rounded-xl hover:from-orange-600 hover:to-red-600 transition-all shadow-lg hover:shadow-xl hover:scale-105">
          Apply Filters
        </button>
      </div>
    </>
